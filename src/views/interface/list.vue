  ...
  <div>
    ...
    <Modal v-model="modalSetting.show"
      ...
      <Form ref="myForm"
        ...
        <FormItem label="真实类库"
                  prop="apiClass">
          <Input v-model="formItem.apiClass"
                 placeholder="请输入真实类库" />
        </FormItem>
        <FormItem label="接口分组"
                  prop="groupHash">
          <Select v-model="formItem.groupHash"
                  style="width:200px">
            <Option v-for="v in apiGroup"
                    :value="v.hash"
                    :key="v.hash"> {{v.name}} </Option>
          </Select>
          <Button icon="md-refresh"
                  @click="getInterfaceGroups(true)"></Button>
        </FormItem>
        <FormItem label="请求方式"
                  prop="method">
        ...
        <FormItem label="接口映射"
          ...
          <Input style="width: 300px"
                 disabled
                 v-model="formItem.hash" />
          <Tag color="error"
               style="margin-left:5px">系统自动生成，不允许修改</Tag>
        </FormItem>
        <FormItem label="AccessToken"
                  prop="accessToken">
          <Select v-model="formItem.accessToken"
                  style="width:200px">
            <Option :value="0"
                    :key="0"> 忽略Token </Option>
            <Option :value="1"
                    :key="1"> 验证Token </Option>
          </Select>
        </FormItem>
        <FormItem label="用户登录"
                  prop="needLogin">
          <Select v-model="formItem.needLogin"
                  style="width:200px">
            <Option :value="0"
                    :key="0"> 忽略登录 </Option>
            <Option :value="1"
        ...
        <FormItem label="测试模式"
                  prop="isTest">
          <Select v-model="formItem.isTest"
                  style="width:200px">
            <Option :value="0"
                    :key="0"> 生产模式 </Option>
            <Option :value="1"
    ...
    <Modal v-model="confirmRefresh"
      ...
      <div slot="footer">
        <Button type="error"
                size="large"
                long
                @click="refreshRoute">确定刷新</Button>
      </div>
    </Modal>
  </div>
</template>
    ...
    return h('Button', {
        ...
        on: {
            'click': () => {
                ...
                vm.formItem.sql = currentRow.sql;
