  ...
  <div>
      ...
      <i-col span="24">
        <Card>
          <p slot="title"
            ...
            <Button type="warning"
                    @click="alertUpload"
                    icon="md-cloud-upload">上传</Button>
          </p>
          <div>
    ...
    <Modal v-model="modalSetting.show"
      ...
      <Form ref="myForm"
        ...
        <FormItem label="字段名称"
                  prop="fieldName">
          <Input v-model="formItem.fieldName"
                 disabled
                 placeholder="请输入字段名称" />
        </FormItem>
        <FormItem label="数据类型"
                  prop="dataType">
          <Select v-model="formItem.dataType"
                  style="width:200px">
            <Option v-for="(v, i) in tableShow.dataType"
                    :value="i"
                    :key="i"> {{v}}</Option>
          </Select>
        </FormItem>
        ...
        <FormItem label="默认值"
                  prop="default"
                  v-if="formItem.isMust.toString() === '0'">
          <Input disabled
                 style="width: 300px"
                 v-model="formItem.defaults" />
          <Tag color="error"
               style="margin-left:5px">仅在字段非必填的情况下生效</Tag>
        </FormItem>
        <FormItem label="规则细节"
                  prop="range">
          <Input disabled
    ...
    <Modal v-model="uploadModal.show"
           width="668"
           :styles="{top: '30px'}"
           @on-visible-change="doUploadCancel">
      <p slot="header"
         style="color:#2d8cf0;">
        <Icon type="md-information-circle"></Icon>
        <span>上传返回字段</span>
      ...
      <Form ref="uploadForm"
            :rules="uploadValidate"
            :model="uploadItem"
            :label-width="80">
        <FormItem label="数据模板"
                  prop="jsonStr">
          <Input v-model="uploadItem.jsonStr"
                 :autosize="{maxRows: 10}"
                 type="textarea"
      ...
      <div slot="footer">
        ...
        <Button type="primary"
                @click="submitUpload"
                :loading="uploadModal.loading">确定</Button>
      </div>
    </Modal>
  </div>
</template>
